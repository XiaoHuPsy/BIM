function [params,err,predicted]=sdrm_sigma0(observed)
% Function for SDRM with sigma_M and sigma_c as 0.
...
lb = [-1e5*ones(1,length(conf)) -1e5];
ub = [1e5*ones(1,length(conf)) 1e5];
options_pso.InitialSwarm = repmat(params,[1000 1]);

params = particleswarm(@(params) sdrm_sigma0_error(params,observed,which_run,set_params),length(params),lb,ub,options_pso);
params=fminsearch(@sdrm_sigma0_error,params,options,observed,which_run,set_params);
...
options_pso = optimoptions(@particleswarm,'Display','off','InitialSwarm',[]); 
lb = [-1e5*ones(1,length(conf)) -1e5 -1];
ub = [1e5*ones(1,length(conf)) 1e5 1];
options_pso.InitialSwarm = repmat(params,[1000 1]);

params = particleswarm(@(params) sdrm_sigma0_error(params,observed,which_run,set_params),length(params),lb,ub,options_pso);
params=fminsearch(@sdrm_sigma0_error,params,options,observed,which_run,set_params);

[err,predicted]=sdrm_sigma0_error(params,observed,which_run,set_params); % get predicted values
