function [cv_logL,cv_logL1,cv_logL2] = cv_sdrm4c_sigma0(observed_data1,observed_data2,nratings)
% cross_validation analysis for Model4c of SDRM (with sigma_M and sigma_c as 0)
...
for i = 1:size(uniqueTrial,1)
    ...
    params = fit_sdrm4c_sigma0(nTrialMat_tmp(1,:),nC2,nTrialMat_tmp(2,:),nI2);
    
    params1 = params([1:(nratings-1) nratings+1 nratings+3]);
...
for i = 1:size(uniqueTrial,1)
    ...
    % model fitting with training data
    params = fit_sdrm4c_sigma0(nC1,nTrialMat_tmp(1,:),nI1,nTrialMat_tmp(2,:));
    
    params2 = [params(1:(nratings-1))*params(nratings) params([nratings+2 nratings+4])];
